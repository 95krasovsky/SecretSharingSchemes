...
inline void b64_to_ZZ(NTL::ZZ& z, const string& str) {
    ...
    string bytes = base64_decode(negsign ? str.substr(1) : str);
    char * cbytes = new char [bytes.length()+1];
    std::strcpy (cbytes, bytes.c_str());
    
    z = NTL::ZZFromBytes((unsigned char *)cbytes, bytes.length());
    if (negsign) { z = -z; }
}
